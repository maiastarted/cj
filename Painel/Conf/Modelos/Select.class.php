<?php
  ...
  class Select {
    ...
    public function Consulta_Simples($Tabela, $Campos, $Where = NULL, $Complemento = []) {
      ...
      try {
        ...
        if (!empty($Complemento)) {
          $key = 1;
          foreach ($Complemento as $valor) {
            $stmt->bindValue($key, $valor, is_int($valor) ? PDO::PARAM_INT : PDO::PARAM_STR);
            $key++;
          }
        }
