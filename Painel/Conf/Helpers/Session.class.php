...
class Session {
    ...
    private function CheckSession($Cache = null) {
        ...
        if (empty($_SESSION['useronline_HQ'])):
            $this->setTraffic();
            $this->setSession();
            $this->CheckBrowser();
            $this->setUsuario();
    ...
    private function sessionUpdate() {
        $_SESSION['useronline_HQ']['online_endview'] = date('Y-m-d H:i:s', strtotime("+{$this->Cache}minutes"));
        $_SESSION['useronline_HQ']['online_url'] = filter_input(INPUT_SERVER, 'REQUEST_URI', FILTER_DEFAULT);
    }
    ...
    private function CheckBrowser() {
        $this->Browser = $_SESSION['useronline_HQ']['online_agent'];
    ...
    private function setUsuario() {
        $sesOnline = $_SESSION['useronline_HQ'];
    ...
    private function UsuarioUpdate() {
        $ArrOnlone = [
            'online_endview' => $_SESSION['useronline_HQ']['online_endview'],
            'online_url' => $_SESSION['useronline_HQ']['online_url']
        ];

        $userUpdate = new Update;
        $userUpdate->ExeUpdate('ws_siteviews_online', $ArrOnlone, "WHERE online_session = :ses", "ses={$_SESSION['useronline_HQ']['online_session']}");

        if (!$userUpdate->getRowCount()):
            $readSes = new Read;
            $readSes->ExeRead('ws_siteviews_online', 'WHERE online_session = :onses', "onses={$_SESSION['useronline_HQ']['online_session']}");
